...
public class CloudSDKHandler {
    ...
    @Before("execution(public * com.uav.*.api.*.*(com.uav.great.context.enums.version.GatewayManager, ..))")
    ...
    @Before("execution(public * com.uav.*.api.*.*(com.uav.great.context.enums.version.GatewayManager, com.uav.great.context.base.BaseModel+))")
    public void checkRequest(JoinPoint point) {
        Common.validateModel((BaseModel) point.getArgs()[1], (GatewayManager) point.getArgs()[0]);
    }

    @AfterReturning(value = "execution(public com.uav.great.context.response.HttpResultResponse+ com.uav.*.api.*.*(..))", returning = "response")
